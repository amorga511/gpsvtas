...
function gpsactivate(evobj){
    console.log(evobj.checked);
    if(evobj.checked==true){  
        $("#map01").show();        
        $("#form_gps").show();            
        $("#form_ngps").hide();
    }else{
...
function getEventFServer(vDv){
    ...
    mrk = new google.maps.Marker({
        position: { lat: 14.033708, lng: -86.568483 },
        title: "You",
      });
    mrk.setMap(vMap);
    ...
    const cityCircle = new google.maps.Circle({
      ...
      cityCircle.setMap(vMap);
      x=1;
      interv = setInterval(function(){
            cityCircle.setRadius((x/100)*1000);
            if (x==100){
                x=0;
            }
            x++;
...
function showPublicaciones(){
    ...
    for(i=0;i<arrPubData.length;i++){
        ...
        switch(parseInt(arrPubData[i].type))
        {
            ...
            case 101:
                //console.log(101)
                vStrHtml += '<img class="card-img-top" width="100%" src="'+ arrPubData[i].url +'" alt="image not found">';
            break;
            case 102:
                //console.log(102);
        ...
        vStrHtml += '<div style="padding:3px">';
        vStrHtml += '<h4>'+ arrPubData[i].title +'</h4>';
        vStrHtml += '<p class="card-text">'+ arrPubData[i].descript +'</p>';
        vStrHtml += '<span class="cfech">'+ arrPubData[i].fecha +'</span><br>';
        vStrHtml += '</div>';
        vStrHtml += '</div>';
